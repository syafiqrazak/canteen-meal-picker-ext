...
function Basic() {
      ...
      chrome.scripting.executeScript(
        ...
        (results: any) => {
          ...
          if (results && results[0].result) {
            ...
            console.log(results[0].result.menu);
          }
        }
      );
    });
  ...
  const randomPick = () => {
    ...
    if (randomize.length === 0) {
        randomMenus = menus.map((menu, index) => ({
            id: index,
            menu: menu
        }))
    } else {
        randomMenus = randomize;
    }
    const correctIndex = Math.floor(Math.random() * randomMenus.length);
    const correctAnswer = randomMenus[correctIndex].menu;
  ...
  return (
    <div className="App">
      ...
      />
      <br />
      <Button as="input" type="button" value="Pick My Meal" onClick={randomPick} />
      <br />
